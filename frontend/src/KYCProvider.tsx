...
const KYCProvider = () => {
  ...
  async function uploadEncryptedFile(file) {
    ...
    //check the account if not then create new account
     let account = await client.login("zaphod@beeblebrox.galaxy");
     if (!account) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        fileCid = await uploadEncryptedFile(selectedFile);
        setLoading('File uploaded successfully. CID: ' + fileCid);
      } else {
        setDialog('Please select a file.');
        return;
      }
      const transaction = await contract.storeUserData(userAddress, `0x${encryptedAge}`, `0x${encryptedDefiProperty}`, `0x${fileCid}`, { gasLimit: 1000000 });
      setLoading('Sending transaction...');
      await provider.waitForTransaction(transaction.hash);
      setLoading('');
